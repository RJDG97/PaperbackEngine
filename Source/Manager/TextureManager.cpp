...
BYTE* TextureManager::CursorLoad(std::string level_name)
{
    ...
    for (rapidjson::Value::ConstMemberIterator file_it = files_arr.MemberBegin(); file_it != files_arr.MemberEnd(); ++file_it) {
        ...
        std::cout << "Tileset is being loaded : " << tileset_name << std::endl;
        ...
        DEBUG_ASSERT(!(fif == FIF_UNKNOWN), (std::string{ "Format of image file is unknown : " } += tileset_name).c_str());
        ...
        DEBUG_ASSERT(dib, (std::string{ "Failed to load image : " } += tileset_name).c_str());
        ...
        DEBUG_ASSERT(!(bits == 0) || (width == 0) || (height == 0),
            (std::string{ "Bits and/or Width and/or Height of image file is zero : " } += tileset_name).c_str());
        ...
        std::vector<GLuint> image_file_data{ width, height, texobj_hdl };
