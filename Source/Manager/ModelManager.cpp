...
Model* ModelManager::AddTristripsBatchModel(int batch_size, std::string model_name)
{
    ...
    glNamedBufferStorage(vbo_batch_, (sizeof(glm::vec2) * 5 + sizeof(float) * 2 + sizeof(glm::vec3)) * 4 * batch_size,
    ...
    //tint color
    glEnableVertexArrayAttrib(vao_batch_, 7);
    glVertexArrayVertexBuffer(vao_batch_, 7, vbo_batch_, offset, sizeof(glm::vec3));
    glVertexArrayAttribFormat(vao_batch_, 7, 3, GL_FLOAT, GL_FALSE, 0);
    glVertexArrayAttribBinding(vao_batch_, 7, 7);
...
Model* ModelManager::AddTextBatchModel(int batch_size, std::string model_name)
{
    GLuint vao_batch_;
    GLuint vbo_batch_;
    GLuint ebo_batch_;
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        for (int j = 0; j < 4; ++j) {
            ...
            tex_vtx_sent.push_back(tex_vtx[j]);
        }
    ...
    glNamedBufferStorage(vbo_batch_,
                         (sizeof(glm::vec2) * 2 + sizeof(float) * 2 + sizeof(glm::vec3)) * 4 * batch_size,
                         nullptr, GL_DYNAMIC_STORAGE_BIT);

    glNamedBufferSubData(vbo_batch_, sizeof(glm::vec2) * 4 * batch_size,
    ...
    //texture id
    glEnableVertexArrayAttrib(vao_batch_, 2);
    glVertexArrayVertexBuffer(vao_batch_, 2, vbo_batch_, offset, sizeof(float));
    glVertexArrayAttribFormat(vao_batch_, 2, 1, GL_FLOAT, GL_FALSE, 0);
    glVertexArrayAttribBinding(vao_batch_, 2, 2);
    ...
    //color
    glEnableVertexArrayAttrib(vao_batch_, 3);
    glVertexArrayVertexBuffer(vao_batch_, 3, vbo_batch_, offset, sizeof(glm::vec3));
    glVertexArrayAttribFormat(vao_batch_, 3, 3, GL_FLOAT, GL_FALSE, 0);
    glVertexArrayAttribBinding(vao_batch_, 3, 3);
